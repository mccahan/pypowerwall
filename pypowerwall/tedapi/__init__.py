...
from .messages import TEDAPIMessage, StatusMessage, ComponentsMessage, ControllerMessage, FirmwareVersionMessage
...
class TEDAPI:
    ...
    def fetch_from_gw(self, method, cache_key, url: str, payload: str, force=False):
        ...
        if isinstance(payload, TEDAPIMessage):
            payload = payload.get_message().SerializeToString()
    ...
    def get_status(self, force=False):
        ...
        message = StatusMessage(self.din)
        url = f'https://{self.gw_ip}/tedapi/v1'
        
        try:
            r, result_code, err = self.fetch_from_gw('post', 'status', url, message, force)
    ...
    def get_device_controller(self, force=False):
        ...
        message = ControllerMessage(self.din)
        url = f'https://{self.gw_ip}/tedapi/v1'
    ...
    def get_components(self, force=False):
        ...
        # Build Protobuf to fetch config
        message = ComponentsMessage(self.din)
        url = f'https://{self.gw_ip}/tedapi/v1'
